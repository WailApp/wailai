...
const getBotVersion = () => {
    return 'WailAI 2.0 Oméga Powered By WailJSCode 0.5 (07-23-2024)'; // استبدل هذا بالإصدار الفعلي
};
...
// دالة للحصول على الوقت المحلي في بلد معين
const getLocalTimeInCountry = (country) => {
    // الخرائط الزمنية للبلدان بناءً على مناطق زمنية محددة
    const timeZones = {
        ...
        'الهند': 'Asia/Kolkata',
        'الصين': 'Asia/Shanghai'
        // يمكنك إضافة المزيد من البلدان والمناطق الزمنية هنا
    };
    ...
    if (!timeZone) {
        return "لم أتمكن من العثور على منطقة زمنية لهذا البلد.";
    }
    
    let localTime = DateTime.now().setZone(timeZone).toLocaleString(DateTime.DATETIME_FULL);
    return `الوقت المحلي في ${country} هو: ${localTime}`;
};

// مثال على كيفية استخدام الدالة
console.log(getLocalTimeInCountry('فرنسا')); // الوقت المحلي في فرنسا
console.log(getLocalTimeInCountry('أستراليا')); // الوقت المحلي في أستراليا
...
const getChatResponse = async (userText) => {
    ...
    if (userText.includes('وداعا') || userText.includes('مع السلامة')) {
	    ...
	    if (userText.includes('اريد لعب لعبة اختيار رقم') || userText.includes('اختر رقم')) {
